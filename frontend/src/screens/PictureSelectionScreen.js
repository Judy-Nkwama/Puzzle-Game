...
const PictureSelectionScreen = ({
    ...
    const { puzzel_blocks_per_row } = useContext(CreateAppContext);
    ...
    const onCrop = useCallback(async () => {
        ...
        try {
            ...
            await cropHandler(name, value, croppedAreaPixels, rotation, puzzel_blocks_per_row, handleFieldChange);
            handleFieldChange("selected_pic_detais", { rotation, croppedAreaPixels });
        ...
        <Row className="h-100">
            <FormGroup className="h-100 d-flex justify-content-center align-items-center">
                <Button // File browser button has to be hidden. Not deleted from the Dom because we still need it to let the use
                    className={`
                                w-100 position-relative ${!value?.selectedImage?.name ? "d-flex" : "d-none"
                        } align-items-stretch 
...
export const cropHandler = async (
    ...
    const pzlDim = puzzel_blocks_per_row;
    ...
    //-- captured current game state data(skore, hamleSayesi)
    if(!!cur_game_data){
        urlsLinkedList.score = cur_game_data.score;
        urlsLinkedList.hamleNumber = cur_game_data.hamleNumber;
    }
