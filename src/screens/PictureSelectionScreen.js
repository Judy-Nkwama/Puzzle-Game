...
const PictureSelectionScreen = ({
    ...
    const handleFileChange = (event) => {
        ...
        if (!selectedImageData) { //If there no file selectd
            return handleFieldChange(name, value, "Aucun fichier n'a été sélectionné");
        }
        ...
        if (!isFileValidType(["image/png", "image/jpeg", "image/jpg"], selectedImageData)) { //  If the selected file is not jpg, jpeg or png --
            ...
            errMess = "Fichier trop lourd (" + getFormatedFileSize(selectedImageData.size) + "). La taille limite autorisée est de " + (maxKBSise / 1024).toFixed(1) + " MegaBytes";
        }
